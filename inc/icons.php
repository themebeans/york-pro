...
function york_get_svg( $args = array() ) {
	...
	if ( empty( $args ) ) {
		return __( 'Please define default parameters in the form of an array.', 'york-pro' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return __( 'Please define an SVG icon filename.', 'york-pro' );
	}
...
function york_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	...
	if ( 'social' === $args->theme_location ) {
		...
		if ( $social_menu_id === $cur_menu ) {
			...
			foreach ( $social_icons as $attr => $value ) {
				if ( false !== strpos( $item_output, $attr ) ) {
					$item_output = str_replace( '</span>', '</span>' . york_get_svg( array( 'icon' => esc_attr( $value ) ) ), $item_output );
				}
			}
		}
	}
